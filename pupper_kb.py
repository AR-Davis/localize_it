...
- You are in KB mode. You can READ and DESCRIBE the system manual only.
- You CANNOT execute tools, run scripts, open files, make changes, log out, or perform any action.
- If the user asks you to "run", "execute", "start", "stop", "log out", "do", or otherwise perform an action, politely decline and say:
  "I'm in read-only KB mode, so I can't run that for you. Switch to tool-runner mode with `pupper-terminal` if you want me to execute something."
- Do not list fake or invented commands in response to action requests.
...
def main():
    ...
    while True:
        ...
        chunks, label, active_labels = retrieve_with_classifier(user_input, index, classifier, top_k=5)
        if classifier:
            print(f"   Routing: {label} (active: {', '.join(active_labels)})")
        if not chunks:
        ...
        # Feedback loop: ask if routing was helpful
        if sys.stdin.isatty():
            try:
                fb = input("  helpful? [y/n/miss]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                fb = ""
            if fb in ("n", "m", "miss"):
                correct = None
                if fb in ("m", "miss"):
                    print("  Labels: TOOL_LOOKUP SCRIPT_INFO HOUND_INFO MESH_INFO STATUS_REQUEST LEARNING OTHER")
                    correct = input("  correct label: ").strip()
                log_kb_feedback(user_input, label, False, correct)
            elif fb == "y":
                log_kb_feedback(user_input, label, True, None)


def log_kb_feedback(query: str, predicted_label: str, helpful: bool, correct_label: str = None):
    """Save feedback so LOCALIZE_IT can retrain the KB classifier.

    correct_label may be a space-separated list of labels, e.g.
    "TOOL_LOOKUP SCRIPT_INFO". It is stored as a JSON list.
    """
    import json
