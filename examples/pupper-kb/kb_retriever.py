...
def classify_query(query: str, classifier) -> Optional[Dict[str, float]]:
    """Return source-type boosts from the KB classifier, or None."""
    ...
    try:
        ...
        idx = classes.index(primary)

        # Simple source boost map keyed to classifier labels
        boost_map = {
            "TOOL_LOOKUP": {"tinker": 3.0, "skill": 1.5, "script": 0.5},
            "SCRIPT_INFO": {"script": 4.0, "skill": 1.0},
            "HOUND_INFO": {"persona": 5.0, "skill": 2.0, "wake": 0.5, "kennel": 0.5},
            "MESH_INFO": {"grove": 4.0, "script": 1.0, "skill": 1.0},
            "STATUS_REQUEST": {"kennel": 3.0, "corraler": 3.0, "wake": 1.5, "skill": 0.5},
            "LEARNING": {"skill": 2.0, "persona": 2.0, "grove": 2.0, "kennel": 1.5},
        }
        return boost_map.get(primary)
    except Exception:
        return None
...
def retrieve_with_classifier(query: str, index: Dict, classifier,
                             top_k: int = 5) -> Tuple[List[Dict], Optional[str]]:
    """Convenience wrapper that classifies then retrieves."""
    source_boosts = classify_query(query, classifier)
    primary_label = None
    if classifier is not None:
        try:
            primary_label = classifier.predict([query])[0]
        except Exception:
            pass

    allowed_types = ALLOWED_TYPES_BY_LABEL.get(primary_label)
...
if __name__ == "__main__":
    ...
    results, label = retrieve_with_classifier(query, idx, clf, top_k=5)
    if label:
        print(f"[KB classifier: {label}]")
    for r in results:
